...
use App\Http\Requests\Admin\WechatResponseRequest;
...
class WechatResponseController extends Controller
{
	...
	{
		$wechat_responses = WechatResponse::where('wechat_id',$wechat->id)->paginate(config('admin.global.paginate'));
		return view(getThemeView('wechat_response.index'), compact('wechat_responses', 'wechat'));
	}

    public function show(WechatResponse $wechatResponse, Wechat $wechat)
    {
        return view(getThemeView('wechat_response.show'), compact('wechatResponse','wechat'));
    }

	public function create(WechatResponse $wechat_response, Wechat $wechat)
	{
		return view(getThemeView('wechat_response.create_and_edit'), compact('wechat_response','wechat'));
	}
...
//        $this->authorize('update', $wechat_response);
        return view(getThemeView('wechat_response.create_and_edit'), compact('wechat_response','wechat', 'group'));
    }
