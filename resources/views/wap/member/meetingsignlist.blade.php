...
@section('content')
    ...
    <section class="aui-content-padded">
        @if($meeting_signs->count())
            <div class="aui-content aui-margin-b-15">
                <ul class="aui-list aui-list-in">

                    @foreach($meeting_signs as $index=>$meeting)
                        <a href="{{route('wap.meeting_signsdetail',['id'=>$meeting->id])}}">
                            <li class="aui-list-item">
                                <div class="aui-list-item-inner">
                                    <div class="aui-list-item-title">{{$meeting->sign_title}}</div>
...
@stop
@section('js')
    @include(getThemeView('layouts._paginate'),[ 'count' => $meeting_signs->total(), ])
@stop